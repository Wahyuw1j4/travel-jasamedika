    ...
    <DashboardLayout>
        <div class="min-h-[calc(100vh-5rem)] flex items-start justify-center bg-slate-50 py-8 px-4">
            <div class="w-full max-w-3xl">
                <!-- Header -->
                <div class="mb-6">
                    <h1 class="text-2xl font-semibold text-slate-800">Edit Jadwal Travel</h1>
                    <p class="mt-1 text-sm text-slate-500">
                        Ubah informasi jadwal travel berikut.
                    </p>
                </div>
...
const form = useForm({
    ...
    departure_datetime: props.travel.departure_datetime ?? '',
    quota_total: props.travel.quota_total ?? 1,
    price: props.travel.price ?? null,
})
...
// Decide whether to allow auto-generation of the name.
// If the travel has no name, or the existing name already matches the generated pattern
// ("Trip {origin} to {destination}"), allow auto-generation. Otherwise respect the
// existing custom name and don't overwrite it when origin/destination change.
let shouldAutoGenerate = false
if (!props.travel.name) {
    shouldAutoGenerate = true
} else if (props.travel.origin && props.travel.destination && props.travel.name === `Trip ${props.travel.origin} to ${props.travel.destination}`) {
    shouldAutoGenerate = true
}
const nameEdited = ref(!shouldAutoGenerate)
...
function submit() {
    ...
    axios.patch(`/api/travels/${props.travel.id}`, payload, { headers })
        .then(() => {
